...
const StepUserAccess = ({onNext, onBack, stepNumber, totalSteps}: Props) => {
  const { userData, updateUserData, updatePatientData, updateProfessionalData, role } = useRegistrationStore();
  ...
  const validate = () => {
    ...
    if (digits.length < 11 && !userData.email) {
        e.phone = 'Por favor, informe um telefone válido ou outra forma de contato (e-mail)';
    }
    if (!userData.cpf || !validateCPF(userData.cpf)) e.cpf = 'Por favor, informe um CPF válido';
    ...
    <>
      ...
      <div className="space-y-4 md:space-y-5">
        ...
        <div>
          ...
          <input type={showPass ? 'text' : 'password'} className="input-cadus"
          value={confirmSenha}
        ...
        <div>
          ...
          <label className="label-cadus">Telefone / WhatsApp (opcional, caso tenha informado seu e-mail)</label>
          <div className="relative">
        ...
        <button type="button" onClick={() => setAcceptedTerms(!acceptedTerms)}
          ...
          <div className={`w-5 h-5 rounded-md flex-shrink-0 mt-0.5 flex items-center justify-center transition-all ${acceptedTerms ? 'bg-primary' : 'border-2 border-border'}`}>
            {acceptedTerms && <Check size={12} className="text-primary-foreground" />}
          </div>
      ...
      <div className="flex items-center justify-center gap-1.5 mt-3 md:mt-4 text-[11px] md:text-xs text-muted-foreground/50">
          <Lock size={12} />
          <span>Seus dados estão protegidos. <a href="#" className="text-primary underline">Saiba mais.</a></span>
        </div>
